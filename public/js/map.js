var APPLICATION_ID = 'kE2pxUWsJOAz3AxroL9Q';
var APPLICATION_CODE = 'XdSzHBeTmdI4YSQ6pJFfsg';
...
var map = new H.Map(mapContainer,
  ...
  center: {lat:50.407344, lng:30.508850},
  zoom: 17
});
...
var content =  '<div class="content"><strong>' + 'Geocoding Autocomplete'  + '</strong></br>';
    content  += '<br/><input type="text" id="auto-complete" onkeyup="return autoCompleteListener(this, event);"><br/>';
    content  += '<br/><strong>Response:</strong><br/>';
    content  += '<div><pre style="max-height:235px"><code  id="suggestions" style="font-size: small;">' +'{}' + '</code></pre></div></div>';

    suggestionsContainer.innerHTML = content;

/**
 * Show marker on map click
 * @param  {H.Map} map      A HERE Map instance within the application
 */
function setUpClickListener(map) {
    map.addEventListener('tap', function (evt) {
        let coord = map.screenToGeo(evt.currentPointer.viewportX, evt.currentPointer.viewportY);
        map.addObject(new H.map.Marker({lat:coord.lat.toFixed(4), lng:coord.lng.toFixed(4)}));
    });
}
